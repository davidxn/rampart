...
class Project_Compiler {
    ...
    function compile_project($bypass_cache) {
        ...
        if ($lock_file_recent) {
            $this->lg("Found a recent lock file - serving old PK3 for now");
        }
        if (!$pk3_is_current) {
    ...
    function generate_map_wads($catalog) {
        ...
        foreach ($catalog as $map_data) {
            ...
            $this->lg("Reading source WAD for map " . $map_data['map_number'] . " (" . $map_data['map_name'] . ")" . PHP_EOL);
            ...
            foreach ($wad_handler->lumps as $lump) {
                ...
                if (in_array($lump['type'], ['midi', 'ogg', 'mp3']) && !$music_bytes) {
                    ...
                    $this->lg("Music found in lump " . $lump['name'] . " with size " . strlen($music_bytes));
                    continue;
                }
